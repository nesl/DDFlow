...
function init(runtime) {
    console.log("NTGBOX", "init flows");
    console.log("NTGBOX", "load Services");
    distributed.loadServices();
    if (started) {
...
function loadFlows() {
    console.log("NTGBOX", "loadFlows");
    var config;
    return storage.getFlows().then(function(_config) {
        // console.log("NTGBOX", "loadFlows", "got flows", _config);
        config = _config;
...
function load(forceStart) {
    console.log("NTGBOX", "load", forceStart);
...
function setFlows(_config,type,muteLog,forceStart) {
    type = type||"full";
    // console.log("NTGBOX", "setFlows", _config);
...
function start(type,diff,muteLog) {
    // console.log("===============================");
    console.log("NTGBOX", "start flows", type, diff, muteLog);
    // console.log("===============================");
    //dumpActiveNodes();
    var booting = (!type);
    type = type||"full";
    ...
    var id;
    if (type === "full") {
        //this is a full start. create new flows and assign them to activeFlows
        ...
        //this is an update. update and/or create flows based on differences from currently deployed flows
    ...
    //go through the flows and start them. I assume they must have been stopped before this call to start
    for (id in activeFlows) {
        if (activeFlows.hasOwnProperty(id)) {
            //NTGBOX: lets say we just dont start the flows. What goes wrong?
    ...
    //NTGBOX:
    distributed.distribute(activeConfig["flows"], diff, booting);
